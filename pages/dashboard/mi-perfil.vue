  ...
  <NuxtLayout name="dashboard">
    <div class="content">
      ...
      <div class="page-header">
        <div>
          <h1>Mis Proyectos</h1>
          <p class="subtitle">Configura tus proyectos para que el módulo de Mis Match calcule tu compatibilidad con cada fondo.</p>
        </div>
        <button
      ...
      <div v-if="loading" class="sk-list">
        <div v-for="i in 1" :key="i" class="sk-card" style="height:420px;border-radius:16px"></div>
      </div>
      ...
      <template v-else>
        ...
        <form v-if="form" class="form" @submit.prevent="guardar">
          ...
          <!-- Filtros avanzados (solo planes de pago) -->
          ...
          <!-- Plan free: aviso de filtros bloqueados -->
...
const { plan, load: loadPlan, canAddProyecto, filtersAvanzados } = usePlan()
...
const tagInput  = ref('')
const editando  = ref<string | null>(null) // id del proyecto en edición, null = nuevo
...
.content { flex: 1; padding: 2rem 2.5rem; font-family: 'Inter', sans-serif; max-width: 760px; }
...
.subtitle { font-size: 0.875rem; color: #64748b; margin-top: 0.25rem; max-width: 520px; line-height: 1.55; }
...
.empty-state { display: flex; flex-direction: column; align-items: center; padding: 5rem 2rem; text-align: center; gap: 0.75rem; }
