...
async function runDigest(env: Env) {
  ...
  const profiles = await sbGet<{ id: string }[]>(
    env, '/rest/v1/profiles?plan=in.(pro,agencia)&select=id'
  )
...
async function processUser(env: Env, userId: string, log: string[]): Promise<boolean> {
  ...
  for (const alerta of alertas) {
    ...
    const items = await fetchMatches(env, alerta, perfil, desde)
...
// ── Matching query ────────────────────────────────────────────────
async function fetchMatches(env: Env, alerta: any, perfil: any, desde: Date) {
